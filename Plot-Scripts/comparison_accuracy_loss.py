...
def accuracy_compare(qubit, layer, option, size, path):
    ...
    for i in range(layer):
        file_names += [path + f"history_{option}_loss_q{qubit}-l{i+1}.npy"]
...
def accuracy_comparison(qubit, layer, option, size, path):
    ...
    file_names = []
    for i in range(layer):
        file_names += [path + f"history_{option}_accuracy_q{qubit}-l{i+1}.npy"]
    WIDTH = 0.5
...
def gradient_comparison(qubit, layer, size, path):
    ...
    file_names = []
    for i in range(layer):
        file_names += [path + f"abs_grads_q{qubit}-l{i+1}.npy"]
